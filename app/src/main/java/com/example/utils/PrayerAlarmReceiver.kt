    ...
    override fun onReceive(context: Context, intent: Intent) {
        ...
        try {
            ...
            if (config.isEnabled) {
                ...
                if (offsetMinutes < 0) {
                    // Normal notification chime 10m before Adhan with preparation reminder
                    ...
                    // If this is a repeat reminder (e.g. 5 mins later), play short takbeerat instead of repeating the entire 3-minute adhan
                    val effectiveSoundMode = if (offsetMinutes > 0 && soundMode == AdhanSoundMode.FULL_ADHAN) {
                        AdhanSoundMode.SHORT_TAKBIR
                    } else {
                        soundMode
                    }

                    // Start Adhan audio foreground service directly with media control notification
                    ...
                    val playSound = soundMode != AdhanSoundMode.VIBRATE_ONLY
