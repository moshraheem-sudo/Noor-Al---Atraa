...
object PrayerNotificationHelper {
    ...
    const val AUDIBLE_CHANNEL_ID = "prayer_times_audible_channel_v1"
    const val AUDIBLE_CHANNEL_NAME = "تنبيهات مواقيت الصلاة الصوتية"
    const val AUDIBLE_CHANNEL_DESC = "تنبيهات صوتية لمواقيت الصلاة والتذكيرات"
    ...
    const val PRE_ADHAN_CHANNEL_DESC = "تنبيهات عادية تسبق موعد الأذان بـ 10 دقائق للاستعداد والوضوء"
    ...
    fun createNotificationChannel(context: Context) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            ...
            // 2. Audible channel for reminders, non-adhan prayers, or when adhan voice is disabled
            ...
            val audibleChannel = NotificationChannel(
                ...
                description = AUDIBLE_CHANNEL_DESC
                enableLights(true)
                lightColor = Color.CYAN
            ...
            // 3. Pre-Adhan notification channel for 10m before Adhan with standard notification chime
    ...
    fun showPrayerNotification(
        ...
        val targetChannelId = if (playAlertSound) AUDIBLE_CHANNEL_ID else CHANNEL_ID
    ...
    fun showTestNotification(context: Context) {
        ...
        val prayerData = repo.prayerTimesData.value
        val timeFormatted = if (prayerData != null) {
            PrayerCalculator.formatTo12hArabic(prayerData.maghrib)
        } else "06:15 م"

        // 1. Post real notification so user can see and verify it in the notification shade
        ...
        // 2. Play Adhan audio cleanly with the ongoing media controls
